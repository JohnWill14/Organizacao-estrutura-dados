...
short tamanhoRegistro(Pessoa p);
...
void pesistePessoaEmFile(Pessoa p, FILE* file){
    ...
    short tamanho = tamanhoRegistro(p);
    fprintf(file, "%02d", tamanho);
...
short tamanhoRegistro(Pessoa p){
    ...
    length+=strlen(p.nome);
    length+=strlen(p.sobrenome);
    length+=strlen(p.endereco);
    length+=strlen(p.estado);
    length+=strlen(p.cidade);
    length+=strlen(p.cep);
